...
//+------------------------------------------------------------------+
class CMamdaniFuzzySystem : public GenericFuzzySystem
  {
private:
...
public:
   ...
   //--- Methods:
   //+------------------------------------------------------------------+
   //| Get output linguistic variables                                  |
   //+------------------------------------------------------------------+
   CList *Output()
     {
      //--- return list fuzzy variable
      return (m_output);
     }
   //+------------------------------------------------------------------+
   //| Get fuzzy rules                                                  |
   ...
   ImplicationMethod ImplicationMethod()
     {
      //--- return implication method
      return (m_impl_method);
     }
   //+------------------------------------------------------------------+
   //| Set implication method                                           |
   //+------------------------------------------------------------------+
   ...
   double Defuzzify(IMembershipFunction *mf,const double min,const double max)
     {
      if(m_defuzz_method==CentroidDef)
      ...
      else if(m_defuzz_method==BisectorDef)
        {
         ...
         //+-------------------------------------------------------------------------------------+   
         double Area=0;                  // The area under the function
         int k=50;                       // The function is divided into "k" steps 
         ...
         now=min;
         double result=fabs(Area/2-mf.GetValue(min));
         while(fabs(Area/2-mf.GetValue(now))<=result)
           {
            result=fabs(Area/2-mf.GetValue(now));
            now=now+(max-min)/k;
           }
         delete mf;
         //--- return result
